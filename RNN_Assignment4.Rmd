---
title: "Weather Forcasting using Neural Network"
...
# Recurrent Neural Network Model and Convolution Network
...
### Defining the Validation and Testing steps for Model
...
Also we have used the max pooling and convolution network.
...
### Implementing model on Test set based on the best stored model - An Example

The model will fit on the test set based on the epochs and the steps_per_epochs which suits the best on validation history, as an example I have used 25 epochs and 500 steps per epochs with callbacks as the Reduced learning rate.
...
Therefore, I have tried using the combination for RNN and CNN with layer_lstm, and drop together to build the above model.

Along with training the model it is useful to construct a checkpoints while training the model so that the model does not overfit and we can determine the best checkpoingt at which the model worked the best. Thus varies methods are implemented above like Callback, Reduce learinig rate, and for visualized checkpoint tensorboard command. 
