...
The reviews are cut off after 150 words

```{r}
maxlen <- 150
```

Training dataset is kept for 100 samples
...
```{r}
rbind(Embedding_result, Pretrain_result)
```

As we could see in the Result we could conclude that:

- Train Set: 100 sample
Since we are using very less sample set for traing purpose, the model could not extract the features of the data and thus the performance is not so good.
